...
- **Captures tab audio** using the browser's `chrome.tabCapture` API to obtain a real-time audio stream from the tab you choose to boost.
- **Processes audio locally** inside an extension Offscreen Document. The audio passes through a Web Audio API pipeline (GainNode → optional DynamicsCompressorNode) and is output to your speakers or headphones.
...
No host permissions (`<all_urls>` or site-specific) are requested.
