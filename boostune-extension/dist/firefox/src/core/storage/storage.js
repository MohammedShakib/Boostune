...
export async function getPrefs() {
  try {
    const result = await platformApi.storage.sync.get(PREFS_KEY);
...
export async function savePrefs(prefs) {
  try {
    await platformApi.storage.sync.set({ [PREFS_KEY]: prefs });
  } catch (e) {
...
export async function getSiteVolumes() {
  try {
    const result = await platformApi.storage.sync.get(SITE_VOLUMES_KEY);
...
export async function setSiteVolume(domain, volume) {
  try {
    ...
    map[domain] = volume;
    await platformApi.storage.sync.set({ [SITE_VOLUMES_KEY]: map });
  } catch (e) {
...
export async function clearAllSiteVolumes() {
  try {
    await platformApi.storage.sync.remove(SITE_VOLUMES_KEY);
  } catch (e) {
...
export async function getSessionStates() {
  try {
    const result = await platformApi.storage.session.get(SESSION_KEY);
...
export async function setSessionState(tabId, state) {
  try {
    ...
    await platformApi.storage.session.set({ [SESSION_KEY]: all });
  } catch (e) {
...
export async function clearAllSessionStates() {
  try {
    await platformApi.storage.session.remove(SESSION_KEY);
  } catch (e) {
